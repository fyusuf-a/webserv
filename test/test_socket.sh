...
PORT=8080
diff <(valgrind --suppressions=memcheck.log --show-leak-kinds=all --leak-check=full --error-exitcode=1 ./a.out $PORT & sleep 4; echo "salut" | nc -w0 localhost $PORT) <(echo "SALUT")
wait $!
