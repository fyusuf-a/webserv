...
curl -v -X POST  --data-binary  @"random1.txt" -H "Transfer-Encoding: chunked" http://localhost:8001/random-output1.txt
# \\ Request 2 // 
dd if=/dev/random of=random2.txt bs=1 count=10000000
curl -v -X POST  --data-binary  @"random2.txt" -H "Transfer-Encoding: chunked" http://localhost:8001/random-output2.txt
# \\ Request 3 // 
yes this is a test | head -n 250000 > random3.txt
curl -v -X POST  --data-binary  @"random3.txt" -H "Transfer-Encoding: chunked" http://localhost:8001/random-output3.txt
